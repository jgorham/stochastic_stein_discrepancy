...
#####################################
# Comparison Plots for likelihood n #
#####################################
dir <- "mnist_7_or_9_sgfs"
distname <- "logisticgaussianprior"
n <- NULL
...
cmp.df <- ldply(c(sgfsd.dat, sgfsf.dat), function(res) {
    sampler <- res$sampler
    data.frame(
        n=res$n,
        discrepancy=sqrt(res$discrepancy2),
        likelihoodn=res$likelihoodn,
        sampler=paste0("Sampler = ", sampler)
    )
})

likelihoodn.vals <- sort(unique(cmp.df$likelihoodn), decreasing=TRUE)
# want 0 at the top
likelihoodn.vals <- c(0, head(likelihoodn.vals, -1))
cmp.df$likelihoodn <- factor(
    cmp.df$likelihoodn,
    levels=likelihoodn.vals,
    labels=c("10000 (all)", as.char(tail(likelihoodn.vals, -1)))
)
...
cmp.plot <-
  ggplot(data=cmp.df, aes(x=n, y=discrepancy)) +
  geom_point(aes(color=likelihoodn, shape=likelihoodn), size=1) +
  geom_path(aes(color=likelihoodn, linetype=likelihoodn)) +
  facet_wrap(~ sampler) +
  scale_x_log10(
      breaks=x.breaks,
      labels=x.labels
  ) +
  labs(x="Number of sample points, n", y="SKSD") +
  guides(
    color = guide_legend(title="Likelihoods, m"),
    shape = guide_legend(title="Likelihoods, m"),
    linetype = guide_legend(title="Likelihoods, m")
  ) +
  theme_bw() +
  theme(plot.margin = unit(c(0,0,0,0.02), "npc"),
        strip.background = element_rect(fill="white"),
...
filename <- sprintf(
    "../../results/%s/figs/julia_%s_dataset=%s_stochastic.pdf",
    dir, dir, dataset
)
makeDirIfMissing(filename)

# Save to file
pdf(file=filename, width=8.7, height=2.7)
grid.arrange(
  marginal.plots, cmp.plot,
  widths = unit.c(unit(0.45, "npc"), unit(0.53, "npc")),
  nrow=1)
dev.off()
