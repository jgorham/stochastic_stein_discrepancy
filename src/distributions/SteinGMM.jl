...
function graddensity(dist::SteinGMM, x::Array{Float64,1})
    d = numdimensions(dist)
    K = length(dist.w)
    runninggraddensity = zeros(d)
    for kk in 1:K
        residual = vec(dist.mus[kk,:]) - x
        sigmakinv = dist.precisions[:,:,kk]
        offmean = sigmakinv * residual
        componentdist = getmixturedist(dist, kk)
        runninggraddensity += dist.w[kk] * offmean * Distributions.pdf(componentdist, x)
    end
    runninggraddensity
end
