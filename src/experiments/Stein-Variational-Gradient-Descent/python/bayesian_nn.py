...
class svgd_bayesnn:
    ...
    '''
        ...
        Input
            ...
            -- batch_size_frac: sub-sampling batch size as a fraction
            ...
            -- use_saga: whether to use SAGA instead of Adagrad for gradient updates
    '''
    def __init__(self, X_train, y_train, X_test=None, y_test=None, batch_size = None, batch_size_frac = None, max_iter = 1000, M = 20, n_hidden = 50, a0 = 1, b0 = 0.1, master_stepsize = 1e-3, auto_corr = 0.9, use_saga = False):
        ...
        log_lik_data_i = -0.5 * (T.log(2*np.pi) - log_gamma) - (T.exp(log_gamma)/2) * T.power(prediction - y, 2)
        f_i = log_lik_data_i + (log_prior_data + log_prior_w) / N
        # log_lik_data = -0.5 * X.shape[0] * (T.log(2*np.pi) - log_gamma) - (T.exp(log_gamma)/2) * T.sum(T.power(prediction - y, 2))
        log_lik_data = T.sum(log_lik_data_i)
        ...
        # compute updates for each individual sample row
        dw_1_i, db_1_i, dw_2_i, db_2_i, d_log_gamma_i, d_log_lambda_i = theano.gradient.jacobian(f_i, [w_1, b_1, w_2, b_2, log_gamma, log_lambda])
        ...
        # automatic gradient for each i
        logp_gradient_i = theano.function(
             inputs = [X, y, w_1, b_1, w_2, b_2, log_gamma, log_lambda, N],
             outputs = [dw_1_i, db_1_i, dw_2_i, db_2_i, d_log_gamma_i, d_log_lambda_i]
        )
        ...
        assert (batch_size is not None) + (batch_size_frac is not None) == 1
        batch_size = batch_size if batch_size is not None else int(N0 * batch_size_frac)
        grad_theta_i = np.zeros([N0, self.M, num_vars])  # stored gradient for SAGA
        ...
        for i in range(self.M):
            ...
            grad_theta_i[:,i,:] = self.pack_weights_vectorized(*logp_gradient_i(X_train, y_train, w1, b1, w2, b2, loggamma, loglambda, N0))
        ...
        for iter in range(max_iter):
            for i in range(self.M):
                ...
                batch = np.random.choice(N0, batch_size, replace=False)
                w1, b1, w2, b2, loggamma, loglambda = self.unpack_weights(self.theta[i,:])
                if use_saga:
                    dw1_i, db1_i, dw2_i, db2_i, dloggamma_i, dloglambda_i = logp_gradient_i(X_train[batch,:], y_train[batch], w1, b1, w2, b2, loggamma, loglambda, N0)
                    grad_theta_batch = self.pack_weights_vectorized(dw1_i, db1_i, dw2_i, db2_i, dloggamma_i, dloglambda_i)
                    grad_theta[i,:] = (1 / batch_size) * (np.sum(grad_theta_batch, axis=0) - np.sum(grad_theta_i[batch,i,:], axis=0)) + np.mean(grad_theta_i[:,i,:], axis=0)
                    grad_theta_i[batch,i,:] = grad_theta_batch
                else:
            ...
            grad_theta_len = np.linalg.norm(grad_theta)

            if use_saga:
                self.theta = self.theta + master_stepsize * grad_theta
            else:
                # adagrad 
                if iter == 0:
            ...
            # save checkpoints
            if X_test is not None and y_test is not None and iter % 10 == 9:
                svgd_rmse, svgd_ll = self.evaluation(X_test, y_test)
                checkpoints.append((iter + 1, svgd_rmse, svgd_ll, grad_theta_len, time.time() - start_time))
    ...
    '''
        Pack all parameters in our model, in a fast vectorized matter
    '''
    def pack_weights_vectorized(self, w1, b1, w2, b2, loggamma, loglambda):
        params = np.concatenate([
            w1.reshape(w1.shape[0], -1),
            b1,
            w2,
            b2[:,np.newaxis],
            loggamma[:,np.newaxis],
            loglambda[:,np.newaxis]
        ], axis=1)
        return params
